...
-----------------------------------------------------------------------*/
jQuery(document).ready(function () {
    const waitlist_arr = $("#waitlist_arr").val();
    if(waitlist_arr) {
      const waitlist_data = JSON.parse(waitlist_arr);
      jQuery('#step_1_tab, #step_2_tab, #step_3_tab, #step_4_tab').addClass('done');
      jQuery('#step_5_tab').addClass('active done');
      jQuery('#step_1_tab, #step_2_tab, #step_3_tab, #step_4_tab').removeClass('active');
      $('#step_6').removeClass('appointment-tab-content');
      jQuery('#step_6').css('display','');
      jQuery('#step_5').css('display', 'none');
      jQuery('#step_1, #step_2, #step_3, #step_4').css('display', 'none');
      let b_dtObj = new Date(waitlist_data.booking_date);
      let format_dt = b_dtObj.toLocaleDateString('en-US', {
        year: 'numeric',
        month: 'long',
        day: 'numeric'
      });
      $('#booking_dt_fnl').html(format_dt);
      $('#booking_slots_h6_fnl').html(waitlist_data.time_slots);
      $('#price_lbl_fnl').html(`$ ${waitlist_data.total_amount}`);
      
      let service_htm = '';
      for(let key in waitlist_data.services_dtls) {
        service_htm += `<tr><td class="p-0 border-0"><h6 class="mb-2">${waitlist_data.services_dtls[key]['service_name']}</h6></td><td class="p-0 border-0"><p class="mb-2 ps-2 text-start">$ ${waitlist_data.services_dtls[key]['price']}</p></td></tr>`;
      }
      $('#show_choose_services_fnl').html(service_htm);
      $('button[name="Previous"]').prop('disabled', true);
    }
    else {
      // jQuery('#step_1_tab, #step_2_tab, #step_3_tab, #step_4_tab').removeClass('done');
      // jQuery('#step_5_tab').removeClass('active');

      // jQuery('#step_2, #step_3, #step_4, #step_5').css('display', 'none');
      // jQuery('#step_1').removeAttr('style');
      // $('button[name="Previous"]').prop('disabled', false);
    }
  // hidden things
  ...
  jQuery(".next").on({
    click: function () {
      ...
      if(!appointment_dt  && $('#step_2_tab').hasClass('active')) {
        Swal.fire({
              // icon: 'error',
              // title: '<span style="color:#ff6d9e;">!</span> <span style="font-size: 24px;">Please Wait</span> <span style="color:#ff6d9e;">!</span>',
      ...
      /*let check_val = $('#customer').val();
      if(check_val === '' && $('#step_4_tab').hasClass('active')) {
        Swal.fire({
            icon: 'error',
            title: 'Error!',
