...
var require_plugin = __commonJS({
  "node_modules/tinymce/plugins/link/plugin.js"() {
    (function() {
      ...
      const isType = (type) => (value2) => typeOf(value2) === type;
      const isSimpleType = (type) => (value2) => typeof value2 === type;
      const eq = (t) => (a) => t === a;
      ...
      const register$1 = (editor) => {
        ...
        registerOption("link_assume_external_targets", {
          processor: (value2) => {
            const valid = isString(value2) || isBoolean(value2);
            if (valid) {
              if (value2 === true) {
                return { value: 1, valid };
              } else if (value2 === "http" || value2 === "https") {
                return { value: value2, valid };
              } else {
        ...
        registerOption("link_list", {
          processor: (value2) => isString(value2) || isFunction(value2) || isArrayOf(value2, isObject)
        });
        ...
        registerOption("link_target_list", {
          processor: (value2) => isBoolean(value2) || isArrayOf(value2, isObject),
          default: true
        });
      ...
      const applyRelTargetRules = (rel, isUnsafe) => {
        ...
        const toString = (rels2) => global$2.trim(rels2.sort().join(" "));
        const addTargetRules = (rels2) => {
          rels2 = removeTargetRules(rels2);
          return rels2.length > 0 ? rels2.concat(rules) : rules;
        };
        const removeTargetRules = (rels2) => rels2.filter((val) => global$2.inArray(rules, val) === -1);
      ...
      const blobToDataUri = (blob) => new Promise((resolve, reject) => {
        ...
        reader.onerror = () => {
          var _a;
          reject(new Error(((_a = reader.error) == null ? void 0 : _a.message) ?? "Failed to convert blob to a data url"));
        };
      ...
      const getLinkAttrs = (data) => {
        ...
        return foldl(attrs, (acc, key) => {
          data[key].each((value2) => {
            acc[key] = value2.length > 0 ? value2 : null;
          });
          return acc;
        }, {
          href: data.href
      ...
      const unlinkImageFigure = (editor, fig) => {
        ...
        if (img) {
          ...
          if (a) {
            (_a = a.parentNode) == null ? void 0 : _a.insertBefore(img, a);
            editor.dom.remove(a);
          }
        }
      };
      const linkImageFigure = (dom, fig, attrs) => {
        ...
        if (img) {
          ...
          (_a = img.parentNode) == null ? void 0 : _a.insertBefore(a, img);
          a.appendChild(img);
        }
      };
      ...
      const sanitizeList = (list, extractValue) => {
        ...
        global$2.each(list, (item) => {
          ...
          if (item.menu !== void 0) {
            ...
            const value2 = extractValue(item);
            out.push({ text, value: value2 });
          }
        });
        return out;
      };
      ...
      const findTextByValue = (value2, catalog) => findMap(catalog, (item) => {
        if (isListGroup(item)) {
          return findTextByValue(value2, item.items);
        } else {
          return someIf(item.value === value2, item);
        }
      });
      const getDelta = (persistentText, fieldName, catalog, data) => {
        const value2 = data[fieldName];
        const hasPersistentText = persistentText.length > 0;
        return value2 !== void 0 ? findTextByValue(value2, catalog).map((i) => ({
          url: {
            value: i.value,
            meta: {
      ...
      const init = (initialData, linkCatalog) => {
        ...
        const getTitleFromUrlChange = (url) => {
          var _a;
          return someIf(persistentData.title.length <= 0, Optional.from((_a = url.meta) == null ? void 0 : _a.title).getOr(""));
        };
        const getTextFromUrlChange = (url) => {
          var _a;
          return someIf(persistentData.text.length <= 0, Optional.from((_a = url.meta) == null ? void 0 : _a.text).getOr(url.value));
        };
      ...
      const handleSubmit = (editor, info) => (api) => {
        ...
        const getChangedValue = (key) => Optional.from(data[key]).filter((value2) => !is(info.anchor[key], value2));
        const changedData = {
          href: data.url.value,
      ...
      const createBlobCache = (editor) => (file, blobUri, dataUrl) => {
        ...
        return editor.editorUpload.blobCache.create({
          blob: file,
          blobUri,
          name: (_a = file.name) == null ? void 0 : _a.replace(/\.[^\.]+$/, ""),
          filename: file.name,
      ...
      const register = (editor) => {
        editor.addCommand("mceLink", (_ui, value2) => {
          if ((value2 == null ? void 0 : value2.dialog) === true || !useQuickLink(editor)) {
            open(editor);
          } else {
      ...
      const setup$1 = (editor) => {
        ...
        const gotoSelectedLink = () => getSelectedLink().each((link2) => gotoLink(editor, link2));
      ...
      const setupButtons = (editor, openLink2) => {
        ...
        editor.ui.registry.addButton("openlink", {
          icon: "new-tab",
          tooltip: "Open link",
          onAction: openLink2.gotoSelectedLink,
      ...
      const setupMenuItems = (editor, openLink2) => {
      ...
      const setupContextToolbars = (editor, openLink2) => {
          ...
          commands: [
            {
              ...
              onAction: (formApi) => {
                const value2 = formApi.getValue();
                const text = getLinkText(value2);
                const attachState = { href: value2, attach: noop };
      ...
      const setup = (editor) => {
        const openLink2 = setup$1(editor);
        setupButtons(editor, openLink2);
        setupMenuItems(editor, openLink2);
        setupContextMenu(editor);
        setupContextToolbars(editor, openLink2);
      };
      var Plugin = () => {
